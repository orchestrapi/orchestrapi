...
class GitClient(ShellClient):
    ...
    def update(git_name):
        ...
        template = [
            'git', '-C', f'{settings.GIT_PROJECTS_ROUTE}/{git_name}', 'pull', 'origin', 'master']
    ...
    def clone(app):
        """Clone the app repo."""
        template = [
            'git', '-C', f'{settings.GIT_PROJECTS_ROUTE}', 'clone', f'{app.git.get("url")}']
        GitClient.call(template)
    ...
    def checkout_tag(git_name, tag):
        ...
        template = [
            'git', '-C', f'{settings.GIT_PROJECTS_ROUTE}/{git_name}', 'checkout', vtag]
        GitClient.call(template)
        template = [
            'git', '-C', f'{settings.GIT_PROJECTS_ROUTE}/{git_name}', 'pull', 'origin', vtag]
        GitClient.call(template)
    ...
    def checkout_master_and_update(git_name):
        ...
        template = [
            'git', '-C', f'{settings.GIT_PROJECTS_ROUTE}/{git_name}', 'checkout', 'master']
        GitClient.call(template)
    ...
    def fetch(git_name):
        """Fetch all branches."""
        template = ['git', '-C',
                    f'{settings.GIT_PROJECTS_ROUTE}/{git_name}', 'fetch']
        GitClient.call(template)
